				...
				<%for(CategoryDto categoryDto : categoryList){ %>
											<form action="list_Board.jsp?user_id=<%=user_id%>&page=1" method="post">
												<button type="submit" name="category"
													value="<%= categoryDto.getCategory2() %>">
													<%= categoryDto.getCategory2() %>
												</button>
											</form>
											<form action="list_Board.jsp?user_id=<%=user_id%>&page=1" method="post">
												<button type="submit" name="category"
													value="<%= categoryDto.getCategory3() %>">
													<%= categoryDto.getCategory3() %>
												</button>
											</form>
											<form action="list_Board.jsp?user_id=<%=user_id%>&page=1" method="post">
												<button type="submit" name="category"
													value="<%= categoryDto.getCategory4() %>">
													<%= categoryDto.getCategory4() %>
												</button>
											</form>
											<form action="list_Board.jsp?user_id=<%=user_id%>&page=1" method="post">
												<button type="submit" name="category"
													value="<%= categoryDto.getCategory5() %>">
								...
								<div class="right_button">
									<% if (userId2.equals(userId)) { %>
										<button onclick="location.href='select_Board2.jsp?user_id=<%=userId2%>'">글
											쓰기</button>
										<% } %>
								</div>
											...
											<% imgYes="" ; %>
												<% } %>
													<tr>
														...
														<td class="leftAlign">
															<a
																href="read_Board.jsp?post_no=<%=item.getPost_no()%>&user_id=<%=userId%>">
					...
					<% int pageSize=10; int totalCount=boardDao.page_No(userId); int totalPageCount=(int)
						...
						<div class="page_no_Link">
							<% for (int i=1; i <=totalPageCount; i++) { String pageLink="?page=" + i + "&user_id=" +
								userId; if (category !=null && !category.isEmpty()) { pageLink +="&category=" +
								category; } %>
								<a class="spaceBar" href="<%= response.encodeURL(pageLink) %>">[
									<%= i %>]
								</a>
								<% } %>
						</div>
						...
						<div id="footer">
							...
							<p>법인명 : (주)오컴퍼니 대표자 : 오조원 사업자등록번호 : 961-11-59611 고객센터 : 1544-9611
							</p>
